...
- **DataService** owns one versioned ProfileStore session per Roblox user ID using `MythicLegends_PlayerData_v2`. It reconciles the schema, associates the user ID, handles session termination, and ends the session when the player leaves. Studio uses an isolated mock store by default.
