...
local log = LogUtil.For("DataManager")
...
DataManager.OnLoaded = loadedBindable.Event
DataManager.OnReleased = releasedBindable.Event
...
function DataManager.IsReadOnly(_player: Player): boolean
	return false
end

function DataManager.GetState(player: Player, key: string): any?
	if not DataManager.Load(player) then
		return nil
	end
	local profile = profiles[player]
...
function DataManager.GetSection(player: Player, sectionName: string): { [any]: any }
	assert(sectionName ~= "", "[DataManager.GetSection] sectionName must not be empty")
	assert(DataManager.Load(player), "[DataManager.GetSection] player profile is unavailable")
	local profile = profiles[player]
	assert(profile and profile:IsActive(), "[DataManager.GetSection] profile session is inactive")
...
function DataManager.ModifyState(player: Player, key: string, newValue: any): boolean
	if not DataManager.Load(player) then
		return false
	end
	local profile = profiles[player]
...
function DataManager.MutateSection(
	player: Player,
	sectionName: string,
	mutator: ({ [any]: any }) -> ()
): (boolean, string?)
	...
	local section = DataManager.GetSection(player, sectionName)
...
function DataManager.MarkDirty(player: Player): boolean
	local profile = profiles[player]
	if not profile or not profile:IsActive() then
		return false
	end
...
function DataManager.WipeAsync(player: Player): boolean
	local profile = profiles[player]
	if not profile or not profile:IsActive() then
		return false
	end
	...
	projections[player] = {}
	publishChanges(player, true)
	return DataManager.SaveNow(player)
end
