--!strict
-- ReplicatedStorage/Shared/ArenaBounds
...
function ArenaBounds.Contains(arena: BasePart?, position: Vector3, heightAllowanceStuds: number?): boolean
	...
	local heightAllowance = if type(heightAllowanceStuds) == "number" then math.max(0, heightAllowanceStuds) else 0

	return horizontalDistance <= radius and math.abs(localPosition.Y) <= arena.Size.Y * 0.5 + heightAllowance
end
