...
local ArenaBounds =
	require(game:GetService("ServerScriptService").Services.CombatService.ArenaBounds)
...
describe("ArenaBounds.Contains", function()
	it("uses the transformed circular footprint when polygon attributes are absent", function()
		...
		expect(
			ArenaBounds.Contains(arena, arena.CFrame:PointToWorldSpace(Vector3.new(11, 0, 0)), 0)
		).toBe(false)
	end)
