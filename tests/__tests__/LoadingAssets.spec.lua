...
describe("LoadingAssets", function()
	...
	it(
		...
		function()
			...
			local authoredWorld = folder("World", world)
			local coreDecoration = mesh(
				"Cap",
				model("FloatingIsland", folder("Visuals", model("Arena", authoredWorld))),
				Vector3.zero
			)
			local islands = folder("BaseIslands", authoredWorld)
			local nearbyIsland = model("BaseIsland0", islands)
			mesh("Grass", folder("Collision", nearbyIsland), Vector3.new(1000, 0, 0))
			local nearbyRock = mesh("Rock", nearbyIsland, Vector3.new(1030, -10, 0))
			local remoteIsland = model("BaseIsland1", islands)
			mesh("Grass", folder("Collision", remoteIsland), Vector3.new(-1000, 0, 0))
	...
	it("selects nearby surfaces using geometry bounds instead of distant model pivots", function()
		...
		mesh("HumanoidRootPart", character, Vector3.zero)
		local authoredWorld = folder("World", world)
		local island = model("BaseIsland0", folder("BaseIslands", authoredWorld))
		island.WorldPivot = CFrame.new(3000, 0, 0)
		local grass = mesh("Grass", folder("Collision", island), Vector3.new(150, 0, 0))
